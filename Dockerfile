...
COPY target/bus-ticket-service.war /usr/local/tomcat/webapps/ROOT.war

# Update Tomcat server.xml to listen on port 9090
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 \
    CMD curl -f http://localhost:9090/bus-ticketer/ || exit 1
